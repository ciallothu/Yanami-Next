...
Yanami Next (repository `Yanami-Next`) supports Android & iPhone for the **Komari** server monitoring tool. The Android app uses Material Design 3, and the iPhone app uses SwiftUI.
...
xcodebuild -project apps/iphone/Yanami.xcodeproj -scheme Yanami -configuration Release -sdk iphoneos -destination 'generic/platform=iOS' -derivedDataPath build/ios CODE_SIGNING_ALLOWED=NO CODE_SIGNING_REQUIRED=NO CODE_SIGN_IDENTITY="" DEVELOPMENT_TEAM="" PROVISIONING_PROFILE_SPECIFIER="" MARKETING_VERSION="$(tr -d '[:space:]' < VERSION)" CURRENT_PROJECT_VERSION="$(tr -d '[:space:]' < VERSION_CODE)" build
```
...
| Ktor 3.4.1 | HTTP client + WebSocket (RPC2 protocol) |
...
- Each request receives an immutable, server-specific authentication context and custom-header set; no process-global session interceptor is used.
