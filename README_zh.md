...
**Yanami Next** 支持 Android & iPhone，用于连接 [Komari](https://github.com/komari-monitor/komari) 服务器监控工具。仓库名为 `Yanami-Next`；Android 端采用 Material Design 3，iPhone 端采用 SwiftUI。
...
- **节点详情看板** — 负载历史、Ping 趋势、IPv4/IPv6、网络速率、区间 Traffic 与累计流量用量
- **SSH 终端** — 双端原生终端引擎，支持回滚、选择、组合输入、特殊键和 WebSocket 尺寸同步
- **桌面小部件** — 基于 Glance 的节点总览桌面小部件，支持刷新和更新间隔配置
- **iPhone App** — 原生 SwiftUI 应用，支持多实例、密码 / API Key / 游客模式、自定义 HTTP Header、自动刷新的节点列表、节点详情、负载与 Ping 记录
...
Android 构建产物位于 `apps/android/app/build/outputs/apk/`。双端应用版本统一读取根目录 `VERSION`，构建号读取 `VERSION_CODE`；未签名 iPhone IPA 安装到真机前需要使用者自行签名。
...
| Termux terminal-view + terminal-emulator | 0.119.0-beta.3 (`e634d8f`) | 本地固定源码的终端渲染与远程传输 |
...
- 每个请求都会显式携带其所属服务器的不可变认证上下文和自定义请求头，避免并发访问多台服务器时发生凭据串用
